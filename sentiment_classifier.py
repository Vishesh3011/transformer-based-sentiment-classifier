...
def get_positional_encoding(emb_size, max_seq_len):
    ...
    # Compute the common numerator and denominator terms
    num = torch.arange(max_seq_len, dtype=torch.float32).unsqueeze(1)
    den = torch.exp(torch.arange(0, emb_size, 2).float() * (-math.log(10000.0) / emb_size))

    # Sin and Cos on alternate dimensions
...
class SentimentClassifier(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size=128, ffn_size=128, num_tfm_layer=2, num_head=2, p_dropout=0.2, max_seq_len=300):
        ...
        # apply layer normalisation before other operations in each transformer encoder layer
        encoder_layer = nn.TransformerEncoderLayer(d_model=emb_size, nhead=num_head, dim_feedforward=ffn_size, dropout=p_dropout)
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_tfm_layer)
    ...
    def forward(self, x):
        ...
        # Get the token embeddings and add the positional encoding
        emb = self.token_embeddings(x)
        PE = self.positional_encoding[:seq_len, :].unsqueeze(1)
        emb += PE
...
def train_model(model, dataset_train, dataset_val, batch_size=64, num_epoch=1, learning_rate=0.001, fmodel='best_model.pth'):
    ...
    training_set = DataLoader(dataset_train, batch_size=batch_size, shuffle=True, collate_fn=collate_fn)
    validation_set = DataLoader(dataset_val, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)
    best_acc = 0.0
    ...
    for epoch in range(num_epoch):
        ...
        # Perform SGD over each mini-batch
        for b_idx, (Xb, yb) in enumerate(training_set, 1):
            ...
            curr_loss += loss.item()
            if b_idx % 10 == 0 or b_idx == len(training_set):
                avg_loss = curr_loss / b_idx
                print(f'{epoch + 1:5d} {b_idx:6d} {avg_loss:25.4f}')
        ...
        print(f'End of Epoch {epoch + 1}, Validation Accuracy: {val_acc:.4f}')

        # get the best accuracy on the validation set and save the model
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), fmodel)
            print(f"Saving the best model with Accuracy: {val_acc:.4f}")

    model.load_state_dict(torch.load(fmodel, map_location=DEVICE))
    return model
...
if __name__ == '__main__':
    ...
    clf = train_model(clf, dataset_train, dataset_val, batch_size=8, num_epoch=10, learning_rate=3e-4, fmodel=fmodel)
    print(clf)
